    ...
    <footer className="relative overflow-hidden bg-forest-900 pb-[34px] pt-20 text-[#CFE6D5]">
      ...
      <Container className="relative">
        <div className="grid grid-cols-1 gap-10 sm:grid-cols-2 lg:grid-cols-[1.6fr_1fr_1fr_1.4fr]">
          <div>
            <div className="mb-4 flex items-center gap-3 font-display text-[18px] font-bold text-white">
              <span className="grid h-[38px] w-[38px] flex-none place-items-center rounded-[12px] bg-gradient-to-br from-green-500 to-green-700">
                <Logo className="h-[22px] w-[22px]" />
              </span>
              Agricom Assurance
            </div>
            <p className="max-w-[34ch] text-[14.5px] text-[#9FC2A9]">
          ...
          {FOOTER_COLUMNS.map((column) => (
            <div key={column.heading}>
              <h4 className="mb-[18px] text-sm font-semibold uppercase tracking-[0.1em] text-white">
                {column.heading}
              </h4>
              {column.links.map((link) => (
                <Link
                  ...
                  href={link.href}
                  className="block py-1.5 text-[15px] text-[#9FC2A9] transition-colors hover:text-lime-300"
                >
                  {link.label}
                </Link>
              ))}
        ...
        <div className="mt-[54px] flex flex-wrap justify-between gap-4 border-t border-white/10 pt-6 text-[13.5px] text-[#86A892]">
